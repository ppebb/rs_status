...
pub fn snprintf_wrapper(format: &str, arg: String) -> String {
    let mut buf = vec![0u8; 0];
    let cformat = CString::new(format).unwrap();
    let carg = CString::new(arg).unwrap();
    let clen: usize = unsafe {
        // This will spit out the correct size of the buffer so we don't
...
pub fn string_from_u8_nul_utf8(src: &mut Vec<u8>) -> Result<&str, std::str::Utf8Error> {
    let nul_pos = src.iter().position(|&c| c == b'\0').unwrap_or(src.len());
    src.truncate(nul_pos);
    return std::str::from_utf8(src);
}
